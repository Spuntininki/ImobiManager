...
import { getRevenueTimeline, getRevenueSummary } from "@/services/ownerService";
import { queryKeys } from "@/hooks/queryKeys";

const EMPTY_TIMELINE = [];
const EMPTY_SUMMARY = { total_amount: "0.00", total_payments: 0 };

export function useRevenueTimeline(ownerId, { start_date, end_date }) {
...
export function useRevenueSummary(ownerId, { start_date, end_date }) {
  const range = { start_date, end_date };
  const { data, isLoading, error } = useQuery({
