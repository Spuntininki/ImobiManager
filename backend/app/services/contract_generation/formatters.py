...
declared for the token in the contract template (Option A: model-in). The
...
Passing the whole model keeps the dispatch site in the pipeline uniform — a
single ``FORMATTERS[token](model_obj)`` call — regardless of how many columns
a formatter consumes.

These functions are pure: they perform no I/O and no DB access. They read
attributes off the passed model object, which makes them trivially unit
testable with lightweight fakes (e.g. ``types.SimpleNamespace``).
"""
...
def format_document(doc: OwnerDocument | RenterDocument) -> str:
    ...
    template only selects *which* aliased row to fetch — once the model object is
