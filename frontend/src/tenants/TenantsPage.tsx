...
export function TenantsPage() {
  ...
  async function handleCreate(payload: Record<string, unknown>, documents: Document[], _initialDocuments: Document[], onClose: () => void) {
    try {
  ...
  async function handleUpdate(renterId: number, payload: Record<string, unknown>, documents: Document[], initialDocuments: Document[], onClose: () => void) {
    try {
      ...
      for (const doc of documents) {
        ...
        if (doc.isDeleted && isExisting) {
          await deleteRenterDocument(renterId, doc.id as number);
  ...
  async function handleDelete(renterId: number) {
    try {
      await deleteRenter.mutateAsync({ renterId, ownerId: Number(selectedOwnerId) });
